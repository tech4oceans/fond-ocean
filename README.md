# Ocean Fund

Open project hub for ocean, climate, biodiversity, marine data, education, AI, and partnerships.

> From the ocean of Earth to the ocean of space.

Ocean Fund builds an open research, education, and technology infrastructure around ocean protection, marine ecosystems, public knowledge, and international collaboration. The project links the ocean of Earth with the ocean of space through marine science, satellite data, education, exploration, and long-horizon public imagination. This repository is the project's public workspace: mission, research directions, data registry, outreach materials, roadmap, and contribution paths live here.

> Status: early public stage. The structure is already open for participation, while texts, materials, and public settings are still being refined.

## What This Is

This repository is the main GitHub hub for Ocean Fund. It is meant to look and function like a serious open initiative, not a random dump of notes. Visitors should be able to understand the mission, see the structure, and find a clear way to contribute.

## Why It Exists

The ocean sits at the intersection of climate, biodiversity, education, science diplomacy, public engagement, and long-term planetary resilience. Ocean Fund needs a transparent, multilingual, and collaboration-ready public base where researchers, educators, volunteers, designers, developers, museums, universities, and partner organizations can orient themselves and join the work.

## What Already Exists

| Area | What is already here |
| --- | --- |
| Mission and governance | Public framing, mission drafts, glossary, governance notes |
| Research | Priority directions, topic mapping, future issue backlog |
| Data | Open-source registry, source map, notebook structure |
| Outreach | Partner emails, conference templates, collaboration formats |
| Presentations | Intro, partner, conference, and exhibition deck outlines |
| Project management | Backlog, next 30/90 days, decision log |
| Contribution workflow | Issue templates, PR template, contribution rules |

## Where To Go Next

- Start with [Mission](docs/mission.md)
- See the [Roadmap](docs/roadmap.md)
- Browse [Research Directions](docs/research-directions.md)
- Explore [Data Sources](docs/data-sources.md)
- If you represent an organization, start with [For Partners](public/partners.md)
- Open [Outreach Materials](outreach/README.md)
- Review [Presentations](presentations/README.md)
- Check the [Backlog](project-management/backlog.md)
- Read [Contributing](CONTRIBUTING.md)

## Focus Areas

| Focus area | Scope |
| --- | --- |
| Ocean research | Biodiversity, climate, marine ecosystems, pollution, bathymetry |
| Data and infrastructure | Dataset registry, source validation, notebooks, visualizations, Earth observation |
| Education and literacy | Public materials, lectures, exhibitions, explainers |
| Ocean technology | Sensors, satellite data, analytics, tooling, AI workflows, ocean-to-space framing |
| Partnerships | Universities, museums, nonprofits, conferences, labs, public institutions |

## How To Participate

1. Read the mission and one research or data document.
2. Pick a small, verifiable improvement.
3. Open an issue or contribute through a pull request.
4. When working with data or claims, include source, access date, license, and verification status.

Good first contributions:

- add one verified open marine data source;
- refine a glossary term or mission paragraph;
- propose a research question;
- improve a partner or conference template;
- clean up repository structure or documentation;
- help prepare the first public-facing GitHub showcase.

## Public Readiness Checklist

The repository should be configured on GitHub with:

- a concise repository description;
- website URL;
- repository topics;
- social preview image;
- enabled Issues;
- enabled Discussions when public idea intake is needed;
- 5-10 public starter issues;
- pinned placement in the `tech4oceans` profile.

See [GitHub Showcase](docs/github-showcase.md) for the full setup standard.

## Partnership Notes

The outreach section is prepared for universities, museums, research centers, nonprofits, conferences, and cross-sector collaborators. Public issues and docs should never include private phone numbers, personal emails, financial details, or unconfirmed partnership claims.

Recommended public path for institutional visitors:

1. Read [For Partners](public/partners.md).
2. Review [Partnerships](docs/partners.md).
3. Use Discussions for open exploration or the `Partner lead` issue template for a concrete, trackable next step.

Current public contact placeholders:

- Email: `partners@example.org`
- Website: `https://example.org`
- GitHub: this repository

These placeholders must be replaced only after official approval.

## Publicity Rules

- Do not publish tokens, credentials, private chats, personal documents, or financial details.
- Do not present unverified partnerships, grants, or operational claims as facts.
- Separate drafts, validated materials, and public-ready outputs.
- Use external platforms as sources or partner references, not as the foundation of project identity.

## License

The final licensing model for text, data, and code is still being defined. Until then, external reuse should be treated as subject to explicit approval.
